...
class ModelPaths:
    ...
    """
    root_dir: str  ## Directory containing all _model components.
    simulink_native: str  ## path to the directory containing stuff
    #  generated by simulink for every _model
    root_model_path: str  ## Path to the root _model
    root_model_name: str  ## Name of the root _model
    compile_type: str  ## grt, ert, etc...
    suffix: str  ## Suffix added to the _model name. Usually 'rtw'
    has_references: bool  ## If this _model contains references
    ...
    def __init__(
        ...
        Args:
            ...
            model_name: Name of the root _model.
            compile_type: grt, ert, etc...
            suffix: the suffix added to the _model name directory. usually 'rtw'
        ...
        for (cur_path, folders, _) in walk:
            ...
            raise RuntimeError(f"{root_dir} is not a valid simulink _model.")
    ...
    def verify_capi(self):
        """
        Make sure that this _model was generated with the c api. This doesn't use
        the function in the capi, but we need the _model mapping interface (mmi).
        """
        files = glob.glob(self.root_model_path + "/*.c", recursive=False)
        files = map(os.path.basename, files)
        assert self.root_model_name + "_capi.c" in files, (
            "Model not generated with capi. Enable the following options in the Code Generation _model settings: \n"
            "\tGenerate C API for: signals, parameters, states, root-level I/O"
        )

        ## also check that this is not a multitasked _model
        with open(os.path.join(self.root_model_path, self.root_model_name + ".h"), encoding="utf-8") as f:
            lines = f.readlines()

        regex = re.compile(f"extern void {self.root_model_name}_step\(void\);") # pylint: disable=W1401
        for line in lines:
    ...
    def compiler_factory(self) -> "anno.Compiler":
        ...
        if self.has_references: # pylint: disable=R1705
            from pysimlink.lib.compilers.model_ref_compiler import ModelRefCompiler # pylint: disable=C0415

            return ModelRefCompiler(self)
        else:
            from pysimlink.lib.compilers.one_shot_compiler import NoRefCompiler # pylint: disable=C0415
